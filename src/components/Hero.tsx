...
// TODO Change margins between title and info (author, views, date). Remember to also update skeleton

export default function Component({ post, className }: Props) {
  return (
    <Link
      href={`/post/${post.slug}`}
      className={`animate-fadeIn group hidden max-w-3xl flex-col gap-3 rounded-lg transition-transform duration-200 active:scale-[0.98] 5xs:flex ${className}`}
